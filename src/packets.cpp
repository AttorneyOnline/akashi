...
void AOClient::pktDefault(AreaData* area, int argc, QStringList argv,
                          AOPacket packet)
{
...
void AOClient::pktHardwareId(AreaData* area, int argc, QStringList argv,
                             AOPacket packet)
{
    setHwid(argv[0]);
    ...
    sendPacket("ID",
               {"271828", "akashi", QCoreApplication::applicationVersion()});
}

void AOClient::pktSoftwareId(AreaData* area, int argc, QStringList argv,
                             AOPacket packet)
{
    ...
    QStringList feature_list = {
        ...
        "cccc_ic_support",  "arup",        "casing_alerts", "modcall_reason",
...
void AOClient::pktBeginLoad(AreaData* area, int argc, QStringList argv,
                            AOPacket packet)
{
    // TODO: add user configurable content
    // For testing purposes, we will just send enough to get things working
    sendPacket("SI", {QString::number(server->characters.length()), "0",
                      QString::number(server->area_names.length() +
                                      server->music_list.length())});
}

void AOClient::pktRequestChars(AreaData* area, int argc, QStringList argv,
                               AOPacket packet)
{
    sendPacket("SC", server->characters);
}

void AOClient::pktRequestMusic(AreaData* area, int argc, QStringList argv,
                               AOPacket packet)
{
    sendPacket("SM", server->area_names + server->music_list);
}

void AOClient::pktLoadingDone(AreaData* area, int argc, QStringList argv,
                              AOPacket packet)
{
    if (getHwid() == "") {
        // No early connecting!
...
void AOClient::pktCharPassword(AreaData* area, int argc, QStringList argv,
                               AOPacket packet)
{
    password = argv[0];
}

void AOClient::pktSelectChar(AreaData* area, int argc, QStringList argv,
                             AOPacket packet)
{
    bool argument_ok;
...
void AOClient::pktIcChat(AreaData* area, int argc, QStringList argv,
                         AOPacket packet)
{
...
void AOClient::pktOocChat(AreaData* area, int argc, QStringList argv,
                          AOPacket packet)
{
    ooc_name = argv[0];
    if (argv[1].at(0) == '/') {
        QStringList cmd_argv =
            argv[1].split(" ", QString::SplitBehavior::SkipEmptyParts);
...
void AOClient::pktPing(AreaData* area, int argc, QStringList argv,
                       AOPacket packet)
{
...
void AOClient::pktChangeMusic(AreaData* area, int argc, QStringList argv,
                              AOPacket packet)
{
...
void AOClient::pktWtCe(AreaData* area, int argc, QStringList argv,
                       AOPacket packet)
{
...
void AOClient::pktHpBar(AreaData* area, int argc, QStringList argv,
                        AOPacket packet)
{
    if (argv[0] == "1") {
    ...
    server->broadcast(AOPacket("HP", {"1", QString::number(area->def_hp)}),
                      area->index);
    server->broadcast(AOPacket("HP", {"2", QString::number(area->pro_hp)}),
                      area->index);
}

void AOClient::pktWebSocketIp(AreaData* area, int argc, QStringList argv,
                              AOPacket packet)
{
    ...
    if (remote_ip.isLoopback()) {
        if (server->db_manager->isIPBanned(QHostAddress(argv[0]))) {
            sendPacket("BD", {server->db_manager->getBanReason(
                                 QHostAddress(argv[0]))});
            socket->close();
            return;
        }
...
AOPacket AOClient::validateIcPacket(AOPacket packet)
{
    ...
    // emote modifier
    // Now, gather round, y'all. Here is a story that is truly a microcosm of
    // the AO dev experience. If this value is a 4, it will crash the client.
    // Why? Who knows, but it does. Now here is the kicker: in certain versions,
    // the client would incorrectly send a 4 here For a long time, by
    // configuring the client to do a zoom with a preanim, it would send 4 This
    // would crash everyone else's client, and the feature had to be disabled
    // But, for some reason, nobody traced the cause of this issue for many many
    // years. The serverside fix is needed to ensure invalid values are not
    // sent, because the client sucks
    ...
    if (text_color != 0 && text_color != 1 && text_color != 2 &&
        text_color != 3 && text_color != 4 && text_color != 5 &&
        text_color != 6)
        return invalid;
