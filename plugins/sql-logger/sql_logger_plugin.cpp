...
bool SqlLoggerPlugin::load(akashi::ServiceRegistry &services)
{
    ...
    if (!l_fs || !l_log || !l_config) {
        qWarning() << "sql-logger: required services not available";
        return false;
    }
    ...
    l_config->declarePlugin(id(), {
        akashi::ConfigEntry(QStringLiteral("db_path"), l_default_path,
    ...
    if (!l_resolved) {
        qWarning() << "sql-logger: path escapes server root:" << l_db_path;
        return false;
    }
