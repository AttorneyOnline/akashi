...
void tst_World::rulesApplyToTheirScopeOnly()
{
    ...
    l_rules.registerAreaRule("no-chat-in-area-2", akashi::AreaEvent::MessageSent, 2,
    ...
    l_rules.registerFloorRule("no-evidence-on-floor-1", akashi::AreaEvent::EvidencePresented, 1,
    ...
    QVERIFY(!l_rules.check(akashi::AreaEvent::MessageSent, {4, 2, 0, "hello"}).allowed);
    QVERIFY(l_rules.check(akashi::AreaEvent::MessageSent, {4, 3, 0, "hello"}).allowed);
    // The floor rule covers every area of its floor, and only that floor.
    QVERIFY(!l_rules.check(akashi::AreaEvent::EvidencePresented, {4, 9, 1, "Knife"}).allowed);
    QVERIFY(l_rules.check(akashi::AreaEvent::EvidencePresented, {4, 9, 0, "Knife"}).allowed);
    // Other events pass through untouched.
    QVERIFY(l_rules.check(akashi::AreaEvent::PlayerJoined, {4, 2, 0, {}}).allowed);
}
...
void tst_World::firstBlockingRuleWins()
{
    ...
    l_rules.registerAreaRule("area-rule", akashi::AreaEvent::PlayerJoined, 0,
                             [](const akashi::AreaEventDetails &) { return akashi::RuleVerdict{false, "the area says no"}; },
                             "test");
    l_rules.registerFloorRule("floor-rule", akashi::AreaEvent::PlayerJoined, 0,
                              [](const akashi::AreaEventDetails &) { return akashi::RuleVerdict{false, "the floor says no"}; },
                              "test");

    QCOMPARE(l_rules.check(akashi::AreaEvent::PlayerJoined, {4, 0, 0, {}}).reason, QString("the area says no"));
}
...
void tst_World::areaRulesOverwriteFloorRulesOfTheSameName()
{
    ...
    l_rules.registerFloorRule("no-evidence", akashi::AreaEvent::EvidencePresented, 0,
    ...
    l_rules.registerAreaRule("no-evidence", akashi::AreaEvent::EvidencePresented, 2,
                             [](const akashi::AreaEventDetails &) { return akashi::RuleVerdict{}; },
                             "test");

    QVERIFY(l_rules.check(akashi::AreaEvent::EvidencePresented, {4, 2, 0, "Knife"}).allowed);
    QVERIFY(!l_rules.check(akashi::AreaEvent::EvidencePresented, {4, 3, 0, "Knife"}).allowed);
}

void tst_World::rulesActOnTheWorldAndAllOfThemRun()
{
    akashi::Area l_vault(2, "Vault", 0, 2);
    l_vault.setLockState(akashi::Area::LockState::Locked);

    akashi::AreaRuleRegistry l_rules;
    // A rule is an executor: saying the magic words in the hallway unlocks
    // the vault. The function captures what it operates on.
    l_rules.registerAreaRule("magic-words", akashi::AreaEvent::MessageSent, 1,
                             ...
                             "test");
    // A blocking rule attached to the same hook must not stop the executor.
    bool l_second_ran = false;
    l_rules.registerAreaRule("silence", akashi::AreaEvent::MessageSent, 1,
                             [](const akashi::AreaEventDetails &) { return akashi::RuleVerdict{false, "Silence in the hallway."}; },
                             "test");
    l_rules.registerFloorRule("watcher", akashi::AreaEvent::MessageSent, 0,
                              [&l_second_ran](const akashi::AreaEventDetails &) {
                                  l_second_ran = true;
                                  return akashi::RuleVerdict{};
                              },
                              "test");

    const akashi::RuleVerdict l_verdict = l_rules.check(akashi::AreaEvent::MessageSent, {4, 1, 0, "open sesame"});

    // The message itself is blocked, but every hook ran: the vault opened
    // and the floor's watcher saw the event.
...
{
    akashi::AreaRuleRegistry l_rules;
    l_rules.registerAreaRule("mine", akashi::AreaEvent::PlayerJoined, 0,
                             [](const akashi::AreaEventDetails &) { return akashi::RuleVerdict{false, "no"}; },
                             "plugin-a");
    l_rules.registerFloorRule("also-mine", akashi::AreaEvent::PlayerLeft, 0,
                              [](const akashi::AreaEventDetails &) { return akashi::RuleVerdict{false, "no"}; },
                              "plugin-a");
    l_rules.registerAreaRule("someone-elses", akashi::AreaEvent::PlayerJoined, 0,
    ...
    QCOMPARE(l_rules.check(akashi::AreaEvent::PlayerJoined, {4, 0, 0, {}}).reason, QString("still no"));
    QVERIFY(l_rules.check(akashi::AreaEvent::PlayerLeft, {4, 0, 0, {}}).allowed);
}
